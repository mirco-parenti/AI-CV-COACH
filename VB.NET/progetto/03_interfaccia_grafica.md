...
- **La fascia dei comandi va a capo** *(2026-08-14)*. In fondo a ogni pannello sta una
  ...
  `MinimumSize` fino a 676 px di bottoni sopra altri bottoni. Non si vedeva perché allora
  l'applicazione si apriva massimizzata, ed è rimasto lì per tre tappe, peggiorando a ogni
...
- **Con T9e arrivano le due che non chiedono niente a nessuno** *(2026-08-22)*. La
  **schermata di avvio** (`FinestraAvvio`) copre il montaggio: senza bordi, centrata sullo
  ...
  prima riscrittura.
  *Dal **2026-09-02** il clic vale **ovunque nell'applicazione**, non solo sulla schermata.
  Il difetto era di quelli che si vedono solo usandola: la schermata è un riquadro in mezzo
  a un desktop, e chi ha la mano sul mouse clicca dove sta guardando — cioè il programma
  dietro. Il clic finiva lì, non succedeva niente, e uno splash che resta dov'è quando gli
  clicchi accanto non sembra una schermata da mandare via: sembra un programma piantato. La
  strada è la stessa dell'Invio, e per la stessa ragione — il filtro dei messaggi vede tutto
  il ciclo, un gestore `Click` vede solo i propri controlli. Si prende il **tasto sinistro**
  e solo dentro l'area di lavoro: il destro apre menù contestuali, e il clic sul bordo o
  sulla barra del titolo serve a spostare e ridimensionare. Il clic si **consuma**, come
  l'Invio: il primo gesto toglie di mezzo la schermata, il secondo è quello vero — non deve
  premere un bottone che chi clicca non stava nemmeno guardando.*
  ...
  **pannello del logo** (cap. 03.5).~~ *(Dal **2026-09-01**, su indicazione del tutor, la
  porta è un bottone **«Informazioni su…»** nelle **Impostazioni**, in cima e accanto a
  «Come funziona…»: il pannello del logo ha smesso di essere cliccabile ed è tornato
  un'insegna — cap. 03.5. Le due cose che di lì si raggiungono, «Cerca aggiornamenti» e
  «Copia diagnostica», non potevano restare senza porta.)* *(Dal 2026-08-26 mostra anche il **commit** da cui
...
- ~~**Il pannello si clicca, e apre «Informazioni su…»** *(T9e, 2026-08-22)*. Vale su tutte
  ...
  suggerimento, perché una porta senza maniglia non è una porta.~~ **Il pannello non si
  clicca: è un'insegna** *(2026-09-01, su indicazione del tutor)*. Il gesto è stato tolto,
  e con lui tutto il suo corredo — nessun gestore del clic, nessuna mano sul puntatore,
  nessun suggerimento che inviti a premere. Il pannello torna a essere quel che era per
  undici tappe: il marchio, la versione e il copyright, e nient'altro. **«Informazioni
  su…» non si è persa**: vive nelle **Impostazioni** (3.4, P8), accanto a «Come funziona…»
  — sono le due voci che parlano *del programma* invece che delle scelte che lo governano,
  e stanno insieme in cima alla finestra, prima che le sezioni comincino. Il trasloco non
  è di comodo: dentro «Informazioni su…» vivono «Cerca aggiornamenti» e «Copia
  diagnostica», e togliere la porta senza dargliene un'altra sarebbe stato perderle in
  silenzio.
...
- **La spia del profilo** *(dal 2026-09-02)*: la lucina che dice se quel che si sta
  ...
  sarebbe un'altra. Vive in **cinque** schermate, una per ogni posto in cui quel che si
  guarda ha una provenienza: la colonna «Profilo usato per il match»
  della coda in P1 (subito dopo il Match, perché **del match** parla), sopra il 📄 CV base
  in P2, accanto alle stelle in P4, in cima a **ciascuna** delle due colonne di P6 — due,
  non una, perché chi guarda la lettera guarda la lettera — e, dal **2026-09-08**, sopra
  gli allegati in **P7**: era l'unica schermata in cui i documenti si *consegnano* senza
  che nessuno dicesse da dove vengono, e chi arriva dalla Home a riprendere una bozza di
  ieri in P6 non ci passa affatto. Il **rimedio** si scrive dove il gesto c'è, e cambia
  con la schermata: in P6 il tooltip rosso nomina «Rigenera», che è lì a due centimetri;
  in P7 dipende da **quale** delle due spie parla, che dalla sera dello stesso giorno
  sono due (v. il punto più sotto); nella coda della Home niente, perché quel
  bottone in quella schermata non c'è, e un avviso vale dove si può fare quel che dice
  *(2026-09-03)*.
- **La spia si ridipinge col suo documento, non un istante prima** *(dal 2026-09-08)*: ogni
  strada che genera, rigenera o salva scrive **prima** la versione e **poi** chiama il
  ridisegno. Detto così sembra una pignoleria d'ordine, ed è invece un difetto visto dal
  vivo: il 📄 CV base rigenerato restava con la lucina di un attimo prima — spenta la prima
  volta, rossa dopo — e tornava onesta solo uscendo dal pannello e rientrandoci. Vale nei
  due versi, perché a disallinearsi sono in due: può cambiare il **documento** sotto la
  spia (P6, «Rigenera») o il **profilo** con cui la si legge (P2, dopo «Salva profilo»).
- **In P7 le spie sono due, perché due sono le cose che escono di casa** *(dal 2026-09-08,
  sera)*. Nata la mattina, la spia dell'email era una sola e stava sopra gli allegati: diceva
  «il 🎯 CV e la ✉️ lettera che stanno per partire» e si accendeva invece guardando se la
  **candidatura** avesse un CV o una lettera — un fatto che con l'elenco lì sotto non
  c'entrava. Bastava una candidatura i cui documenti non fossero mai stati **esportati** —
  che è il caso normale di chi arriva dalla Home — per vederla rossa sopra due file scritti
  quel giorno stesso, con un tooltip che mandava a riesportare roba che lì non c'era.
  Trovata guardando il programma, non il codice.
  - **Il difetto non era il rosso: era il soggetto.** Di qui escono **due** cose, e si
    disallineano ognuna per conto suo. Il **testo**, che nasce dalla ✉️ lettera e da
    nient'altro (cap. 07.1: il compositore non è un quarto generatore, parte da un documento
    già scritto e lo accorcia). E i **file**, che sono quelli esportati, e che rigenerare non
    tocca. Una spia sola doveva per forza mentire su una delle due. Adesso ognuna sta sopra
    la cosa di cui parla — una sotto «Il messaggio», una sotto «Cosa allego» — ed è la stessa
    ragione per cui in P6 le spie sono due e non una in cima al pannello.
  - **Ogni spia nomina il rimedio che alla sua cosa si applica.** Quella del **testo** dice
    due gesti in **due schermate**, e in quest'ordine: rigenerare la lettera con «◀ Torna ai
    documenti», poi tornare qui e premere «Fallo riscrivere» — al contrario si ripartirebbe
    dalla stessa lettera vecchia, e sembrerebbe non aver fatto niente. Quella degli
    **allegati** resta com'era: rigenerare *e riesportare*.
  - **La spia degli allegati guarda l'elenco.** Si accende solo se fra gli allegati c'è un
    documento **della candidatura**: il 📄 CV base viene dal profilo di oggi per definizione
    e gli attestati dal profilo non nascono, quindi a poter essere vecchio è solo il primo. Se
    in elenco non ce n'è nessuno non c'è niente da giudicare, e la spia tace — mentre quella
    del testo continua a dire la sua, perché il messaggio da quella lettera viene comunque.
  - **I bottoni di P7, invece, erano già in regola**, ed è la stessa domanda vista dall'altro
    capo. Il cancello del 2026-09-02 (cap. 12, A7) ferma chi **riscrive un documento partendo
    dal profilo**: qui non lo fa nessuno — «Fallo riscrivere» parte dalla lettera, «Prepara
    l'email» impacchetta quel che c'è, la spunta del 📄 CV base impagina un CV che il profilo
    di oggi ce l'ha per definizione. Vale anzi il verso opposto, ed è scritto in A7: *quel che
    è già scritto si può ancora mandare*. In P7 il compito di dirlo ce l'hanno le spie, non i
    lucchetti.
...
- **I bottoni delle tappe non ancora arrivate si vedono, spenti, con un tooltip che dice
  ...
  *(Deciso in T3c, 2026-08-07.)* *E il **2026-09-01** la regola ha incontrato il caso che non
  aveva previsto: una tappa che **smette di essere promessa**. Col flusso D dichiarato fuori
  dalla 1.0 (cap. 12.4), «Sessione di aggiornamento» in P2 è un bottone spento la cui tappa
  non arriverà più — e il tooltip che diceva in quale tappa sarebbe arrivato sarebbe diventato
  una bugia. La scelta è stata **tenere il bottone e cambiargli la frase**: dice che la
  sessione è «rimandata a una versione futura» e indica dove si fa intanto la stessa cosa (il
  profilo si aggiorna dalla scheda, campo per campo). Toglierlo sarebbe stato più pulito e
  meno onesto: la funzione esiste nel progetto, e un'interfaccia che la nasconde dice che non
  è mai stata pensata. La regola si estende così: **spento con il tooltip che dice quando —
  e, se il quando non c'è più, che dice che non c'è più e dove si arrangia chi la cercava.**
