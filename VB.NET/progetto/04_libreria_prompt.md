...
- **`max_token`** è il tetto della risposta, e si dimensiona sul **contenuto che quel
  ...
  *Dal 2026-08-18 quel dimensionamento si può **misurare** invece che stimare*: ogni
  ...
  prompt erano tarati sul modello di prima. *Misurata il **2026-08-19** su un giro d'uso
  completo — che è l'unico modo di farla, non a tavolino: il più sollecitato dei quindici
  tetti è `email_candidatura` al **27,1%** del proprio, e nessuna delle tredici chiamate si
  è troncata. **Nessun tetto è stato alzato**: i numeri scritti col Pool 1.03 reggono anche
  al modello nuovo, con un margine di quasi 4×. Il limite del dato è dichiarato — è un giro,
  con un CV e un annuncio — e la misura resta lì per il prossimo cambio di modello.*
...
| Testo dei prompt | 15 prompt validati | **identici** alla migrazione (Pool 1.00), salvo adattamento segnaposto; nuovi prompt ✚ progettati con lo stesso metodo. Dal **Pool 1.01** `importa_cv` diverge di proposito, dal **Pool 1.02** anche i sette turni del profilo, e dal **Pool 1.03** `analisi_annuncio`, che estrae anche il nome dell'azienda (v. `CHANGELOG.md`): su quei prompt il prototipo non è più il metro, è il termine di paragone. Il metro carattere-per-carattere resta su `confronto` e `mitigazione`. Il **Pool 1.04** (2026-08-14) è il primo bump che **aggiunge** invece di correggere — i due prompt di T6, `email_candidatura` e `classifica_documenti` — e non tocca nessuno dei quindici: le loro impronte restano quelle di 1.03, e chi confronta i due manifest lo vede subito. Il **Pool 1.05**, poche ore dopo, corregge il primo dei due su una lezione che vale oltre il suo caso: **l'esempio pesa più della regola**, e se i due si contraddicono vince l'esempio. Con T7 il pool cresce in una direzione che il prototipo non aveva: **1.06** e **1.07** gli insegnano l'inglese (le varianti `.en` dei tre documenti, poi dell'email), **1.08** apre `rifinitura/`, e **1.09** ne corregge i tre difetti che solo l'AI vera poteva mostrare. Quella lezione del 1.05 è tornata due volte, sempre più precisa: a 1.07 nella forma «fra una regola e una **forma da imitare** vince la forma», a 1.09 in quella più generale — fra due istruzioni che si contraddicono vince **quella che parla del testo intero**, non la più specifica. Il **1.10** apre `brainstorm/` e fa una cosa che il prototipo non aveva mai fatto: un prompt che è il **primo messaggio di una conversazione**, con uscita a testo e la risposta consegnata a pezzi (cap. 02.5). Il **1.11** fa tornare quella lezione una terza volta, nella sua forma più netta: i due prompt inglesi del CV volevano tradurre il titolo di ruolo — la regola anti-upgrade lo nomina, l'elenco dei nomi propri no — ma accanto al campo c'era un «**Copy**» senza rimando, e in un CV inglese è rimasto un «Direttore Operativo». **Fra un'istruzione concreta accanto al campo e una regola generale in un'altra sezione, vince la concreta**: la cura non aggiunge regole, rimette il rimando dov'era stato dimenticato. Il **1.12** (2026-08-19) chiude la stessa famiglia con il caso più puro: in `contatti` l'istruzione «un corso → formazione» c'era già e perdeva contro la regola scritta accanto, «non aggiungere e non inventare nulla», ogni volta che l'accenno si auto-negava («un corso, ma non ricordo quale né dove») — due regole giuste che si contendono la stessa frase, e a perdere era l'anti-perdita. La cura è una riga che dichiara che **un accenno vale anche quando è incompleto**, messa *prima* dell'altra: quando due istruzioni si contendono un caso, l'**ordine di lettura** è la leva più economica per dire quale viene prima. Il **1.13** (2026-08-23, quinto tempo di T9e) è il primo bump che nasce da un **collaudo dal vivo** invece che da un banco, e tocca tre prompt. In `contatti` la via di un indirizzo spariva in silenzio: non era materiale di un altro turno — quindi `altrove` non la copriva — ma il prompt chiedeva «una città sola» e il modello obbediva. Nasce `lasciato_fuori`, il terzo posto dove finisce **quel che il turno ha scelto di non tenere**, con le parole dell'utente e rimesso davanti prima di confermare: la scelta resta, smette di essere muta. In `esperienze_formali` un mestiere detto nudo («ho fatto lo spazzino») veniva scartato, mentre la regola dell'`altrove` **nello stesso prompt** dice che un accenno vale anche incompleto — la stessa incompletezza si teneva se destinata a un altro turno e si buttava se destinata a questo; e «in un negozio di pittura» finiva in `ruolo` invece che in `azienda`, perché un campo vuoto è un dato onesto e un campo riempito con la cosa sbagliata no. Nella `lettera` (it ed en) entra il settimo blocco `<riscritture>`: fin dal 1.00 il `<cv>` **non** è fonte di fatti — l'ha scritto un modello dal profilo, e farglielo confermare da sé non aggiunge niente — ma da T9d l'utente può riscriverne la prosa a mano, e quelle parole sono una **dichiarazione della persona**, come il profilo. Il blocco porta solo i campi riscritti col loro testo, ed è l'unica cosa dentro un CV che valga come fonte; l'anti-invenzione non è toccata, perché vieta al modello di inventare, non all'utente di dichiarare |
