...
|---|---|---|---|
| 14 | **Come curare i difetti di scala a DPI alto**: il pannello del logo che a 150% sfonda nell'area viva, la finestra Impostazioni irraggiungibile per metà, il minimo della finestra principale sceso sotto la soglia di progetto (cap. 14, giro B) | **(a) puntuale** — si aggiusta ciascuno dei tre punti dove fa male. **(b) unità coerenti** — un modo unico di dichiarare le misure dell'interfaccia rispetto al DPI, di cui i tre punti diventano clienti: più lavoro, ma toglie la **specie** di errore invece dei tre esemplari. In entrambi i casi va deciso **come il banco potrà vedere questi difetti**: oggi gira a 96 DPI, dove i numeri sbagliati sono giusti, e nessuno dei 995 collaudi verdi li mostrava. | ✔ **Decisa il 2026-08-23: la (a), ma collaudabile.** Il censimento del codice ha contato **22 punti** della stessa forma, più un pattern di **~83 somme** di costanti di `StileApp` sparse su 16 file: in astratto un buon argomento per la (b). Non è stata scelta per due ragioni. La prima è che i tre difetti **non erano la stessa specie**, e guardarli da vicino l'ha mostrato: R10a è davvero un confronto fra unità diverse; R10b è una **costante che duplica** una misura che il runtime già possiede, e si toglie invece di convertirla; R11 non è un problema di unità ma di **tetto e scorrimento mancanti** — si sarebbe rotta uguale a 96 DPI su uno schermo alto 768, e infatti `in_sospeso.md` lo temeva da prima del collaudo; R12 è **doppia scalatura**, perché `AutoScaleMode.Font` la `MinimumSize` la scala già da sé, con rapporti diversi nelle due direzioni, e un convertitore in più moltiplicherebbe due volte. Un'unità coerente, da sola, ne avrebbe curato **uno**. La seconda ragione è di rischio: la (b) piena vuol dire rimettere mano al posizionamento di sedici file **senza rete**, perché il banco non ha un solo collaudo di misure d'interfaccia e il layout è quello validato a video a T3 — al quinto tempo su sei. **Attuazione**: un modulo `ScalaSchermo.vb` di **funzioni pure a cui il DPI si passa** (prima non esisteva in tutto il progetto una riga che leggesse il DPI), di cui sono clienti **tre** dei quattro punti — il quarto, l'ingombro del logo, non si converte affatto: si **toglie**, perché il valore giusto ce l'aveva già il runtime; più il tetto e l'`AutoScroll` su **tutte e quattro** le finestre che condividono la riga di R11 — Impostazioni, Backup, ChiaveApi, ConfermaCritica — e non solo su quella che è caduta. Così il banco vede a 96 DPI difetti che vivono a 144: **dieci collaudi nuovi**, tutti falsificati rimettendo sotto i bug originali (regola 14). **Verificato dal vivo a 150%**: pannello logo in compatta 186×160 senza sfondamenti, finestra minima **1725×900** (i 1150×600 di progetto, esatti), Impostazioni **1012×1008** dentro l'area di lavoro e ogni comando raggiungibile scorrendo. Le ~83 somme restano **fuori dalla 1.0**, annotate in `in_sospeso.md`: danno cosmetico, rischio strutturale. |
