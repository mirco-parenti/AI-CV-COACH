...
| ~~Un fondo pieno reagisce al mouse~~ **(superata)** | `TrovaLavoro/StileApp.vb` | ~~si tolgono le due righe `MouseOverBackColor`/`MouseDownBackColor` in fondo a `Dipingi` **e** a `DipingiLaCasella`~~ | *Non più rifacibile dal **2026-09-01**: col passaggio a `FlatStyle.Standard` l'ombreggiatura sotto il puntatore la disegna Windows, le due righe non esistono più e con loro i due collaudi (`OgniBottoneSiScurisceSottoIlPuntatore`, `AncheLeCaselleDellaBarraSiScurisconoSottoIlPuntatore`). Resta scritta perché dice che quella difesa c'era, e perché non c'è più.* |
...
| L'eliminazione di tutto ha una riga tutta sua | `TrovaLavoro/Ui/FinestraImpostazioni.vb` | il critico riallineato a sinistra e il margine sotto rimesso a 14 | `LEliminazioneDiTuttoHaUnaRigaTuttaSua` *(si chiamava `…HaUnaFasciaTuttaSua` fino al 2026-09-01)* |
