...
### Step 2.67 — La colonna che diceva come chiamo le cose io, e adesso dice cosa manca a te

*Due richieste piccole in apparenza: scambiare due colonne, e far dire alla terza a che punto è la candidatura invece di «Interessante» o «Generata». La prima è durata un'ora; la seconda ha scoperto che il dato per rispondere la Home non ce l'aveva proprio.*

**Cosa ho fatto**
- **Ho messo l'azienda ad aprire la coda.** Le prime due colonne erano la spia e il match: un giudizio e la sua qualifica, cioè un punteggio di cui non si era ancora detto il soggetto. Adesso si legge chi offre il posto, poi quanto vale, poi se quel valore è ancora buono. La spia scala in terza posizione e resta quello che era — la lucina del **match**, non dei documenti.
- **La colonna «Stato» ha cambiato mestiere.** Diceva il nome dello stato interno; adesso dice a che punto è la procedura: `CV mirato ✓ · lettera ✓ · email ✓`. Le tre voci ci sono **sempre tutte e tre**, con la spunta o col trattino, perché in un elenco si guarda in giù e in giù si incolonna solo ciò che sta sempre nello stesso posto. Quando i documenti che ci sono non vengono dal profilo di oggi la cella lo dichiara — `⚠ obsoleti` — e va in rosso.
- **Com'è andata è passata in una colonna sua, «Esito»**: Colloquio, Rifiutata, Assunto 🎉, Scartata, e i giorni di silenzio di una spedita ferma da troppo. A sinistra quello che si legge dalla cartella, a destra quello che dichiaro io.
- **Ho dovuto insegnare al registro cosa sono i documenti.** La coda non apre le candidature, legge il `registro.json` — e lì c'erano stato, stelle e versione del confronto, non se il CV e la lettera esistono né con che profilo sono nati. Ora c'è un blocco `documenti`, e un indice che non ce l'ha **si rigenera** invece di essere usato.
- **Le larghezze le ho misurate, non stimate**: la riga più lunga della colonna misura 239 px, la colonna è 250; le cinque colonne intorno hanno restituito i 150 px che servivano, senza che nessun testo si accorci. **1434 verdi**, sette falsificazioni nuove, tutte viste rosse. Poi ho aperto l'applicazione sui dati di prova e ho guardato: Officine Verdi, match rifatto ieri sera, spia verde e Stato rosso.

**Cosa ho imparato**
- **Un campo che manca non si legge sempre allo stesso modo.** Il registro è un riflesso delle cartelle, e la regola scritta a T5c dice che ci si aggiunge una colonna senza migrare niente: chi non ce l'ha la lascia vuota. Vale finché quel vuoto si mostra come **silenzio** — la spia spenta di una versione non annotata. Qui il vuoto si sarebbe mostrato come `CV mirato –`, che non è silenzio: è una risposta, ed è falsa. La differenza fra «no» e «non lo so» non è filosofia, è cosa legge chi guarda.
- **Un ListView colora per cella, non per parola.** Volevo le spunte nere e la coda in rosso: non si può, se non ridisegnando a mano tutta la coda. Ci ho messo un attimo a capire che non era un ripiego — la riga intera dice «questi documenti non valgono più», e le spunte fanno parte di quella frase.
- **«CV» da solo non basta più.** In questo programma i CV sono due, e una colonna che dicesse `CV ✓` sulla riga di un'azienda lascerebbe a chi legge il compito di ricordarsi quale. Sono nove caratteri in più e la colonna è cresciuta di cinquanta px: li ho pagati volentieri.

**Dove ho faticato**
- **Su un collaudo che è caduto dicendo la verità.** Avevo scritto la prova «coi documenti in pari la colonna non aggiunge niente» usando la semina degli altri: solo che là il profilo cambia a metà strada, e in quel mondo **non c'è più niente in pari**. Il rosso non era un difetto del codice, era il collaudo che descriveva una situazione impossibile. Gli ho dato una semina sua, di tre righe.
- **E sulla trappola di sempre di VB**: una funzione d'appoggio che si chiamava `Riga` e tre variabili locali chiamate `riga` che la coprivano — l'errore parla di una classe «che non può essere indicizzata», e non nomina mai la vera causa. Rinominata `RigaDi`, con il perché scritto lì accanto.

**Cosa ho deciso e perché**
- **La spunta dell'email vuol dire «partita», non «bozza pronta»**: a spedire è il mio programma di posta, e l'unica prova che l'applicazione può avere è la mia parola. Una bozza scritta e mai mandata è un trattino, ed è la verità.
